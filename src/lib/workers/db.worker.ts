...
/**
 * Top-level try-catch to capture fatal initialization errors.
 */
try {
  console.log('[DBWorker] Worker script loaded and initializing...');
  ...
  async function getSqlJs() {
    ...
    if (!wasmResp.ok) {
      throw new Error(`WASM fetch failed at ${wasmUrl}: ${wasmResp.status} ${wasmResp.statusText}`);
    }
  ...
  self.onmessage = async (e: MessageEvent) => {
    ...
    try {
      ...
      console.error(`[DBWorker] Error processing message ${type}:`, error);
      self.postMessage({ 
        id, 
        type: 'ERROR', 
  ...
  console.error('[DBWorker] FATAL INITIALIZATION ERROR:', fatalError);
  // @ts-ignore
  self.postMessage({ type: 'ERROR', payload: { message: `FATAL: ${fatalError.message}` } });
}
